...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k < num_rolls:
        ...
        if sided == 1:
            sad = True
        total, k = total + sided, k + 1
    return 1 if sad else total
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    play = player_score if player_score < 10 else player_score % 10
    opponent = 0 if opponent_score < 10 else opponent_score // 10 % 10
    score = 3 * abs(opponent - play)
...
def num_factors(n):
    ...
    计算并返回给定整数 N 的所有正因子的数量。比如，对于数字 6 其因子有 1, 2, 3, 和 6 所以函数应该返回 4
    """
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    num, k = 0, 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    count, hight, best = 1, 0, 0
    make_average = make_averaged(roll_dice, times_called)
    while count < 11:
        score = make_average(count, dice)
        if score > hight:
            hight, best = score, count
        count += 1
    return best
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if boar_score >= 100:
        ...
        return 3
    elif score > 80 and score - opponent_score > 20:
        return 0
    else:
